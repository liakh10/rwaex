...
/* raw JSON-RPC with fallback: the main Robinhood RPC sometimes answers server-side fetches with a Cloudflare
   challenge page instead of JSON */
async function rpc(req) {
  let last;
  for (const url of CHAIN.rpcUrls.default.http) {
    try {
      const text = await (await fetch(url, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify(req) })).text();
