...
function onAccounts(a) { W.address = a && a[0] ? a[0] : null; if (!W.address) { try { localStorage.removeItem('rwaex:wallet'); } catch {} } emit(); }
...
export async function connect(id) {
  ...
  try { localStorage.setItem('rwaex:wallet', d ? d.info.rdns : 'injected'); } catch {}
...
export async function restore() {
  let rdns = null; try { rdns = localStorage.getItem('rwaex:wallet'); } catch {}
...
export function disconnect() {
  W.address = null; try { localStorage.removeItem('rwaex:wallet'); } catch {}
